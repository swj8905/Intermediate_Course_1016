...
if submit_button:
    ...
    while True:
        try:
            ...
            try:
                ...
                reply_of_reply = 1
            # print(user_comment)
            st.write(f"##### {user_comment}")

            score = sentiment_predict(user_comment)
            like_num = string_to_num(like_num)
            df = df.append({"score":score, "like":like_num, "reply":reply_of_reply, "comment":user_comment}, ignore_index=True)
    ...
    st.write("# 감성분석 결과")
    fig = alt.Chart(result).mark_bar().encode(x=alt.X("감성", sort=list(sentiment_result.keys())),
                                        y="댓글 수")
    st.altair_chart(fig, use_container_width=True)

    alt_scale = alt.Scale(domain=[1, df["reply"].max()])
    fig_circle = alt.Chart(df).mark_circle().encode(x="score", y="like",
                                       size=alt.Size("reply", scale=alt_scale),
                                       color="reply", tooltip=["score", "like", "reply", "comment"])
